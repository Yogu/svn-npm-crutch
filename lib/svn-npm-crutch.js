...
// Do not override modules that are linked with "npm link"
function checkIsSymlink(dep, cb) {
  var path = rootDir + "/node_modules/" + dep;
...
async.eachSeries(Object.keys(deps), function(dep, cb) {
  checkIsSymlink(dep, function(error, isSymlink) {
    if (error) {
      return cb(error);
    }
    if (isSymlink) {
      logok("Ignoring dependency " + dep + " because it is installed as symlink");
      return cb();
    }
